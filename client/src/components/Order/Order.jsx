...
export const Order = () => {
    ...
    useEffect(() => {
        ...
        if (stringCart) {
            ...
            const totalAmount = items.reduce((acc, cur) =>
                acc + (Number(cur.Price) * cur.quantity),
                0);
    ...
    return <>
        <Box maxWidth="90%" margin="1em auto">
            <Card>
                <CardContent>
                    <Content customer={customer} cartItems={cartItems.items} totalAmount={cartItems.totalAmount} />
                    <Spacer />
                    {order.status === 'idle' &&
                        <Button color='primary' type="button" onClick={handlePayment} >PAGAR</Button>
                    }
                    ...
                    {order.status === 'failed' &&
                        <>
                            <p>Error al procesar el pedido! Por favor consulte con el administrador del sitio</p>
                        </>
                    }
                </CardContent>
            </Card>
